...
const frameLooper = birdImg => {
  ...
  if (gameStartImgReady && instructionImgReady && gameStarted === false) {
    ...
    ctx.drawImage(gameStartImg, canvas.width / 2 - (gameStartImg.width * 2), 50, 400, 100);
    ctx.drawImage(instructionImg, canvas.width / 2 - (instructionImg.width) - 10, 400, 400, 100);
  }
...
const handleGameOver = () => {
  ...
  if (gameOverImgReady && instructionImgReady) {
    console.log("ready");
    ctx.imageSmoothingEnabled = false;
    ctx.drawImage(gameOverImg, canvas.width/2 - (gameOverImg.width * 2), 50, 400, 100);
    ctx.drawImage(instructionImg, canvas.width / 2 - (instructionImg.width) - 10, 400, 400, 100);
  }
};